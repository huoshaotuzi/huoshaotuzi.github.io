...
在上面的使用场景中，两种方法的结果和过程完全一致，因此用星号查询没有毛病！

面试官要是敢不说使用场景就问你这个问题，就把简历要回来重新甩在他脸上一走了之！

## SELECT * 的优点
看到这里是不是颠覆常规的认知了？

众所周知不论书上还是网上一致都在抵制 SELECT 星号查询，那为什么说这个还能有优点呢？

抛开使用场景不谈而直接讨论命题的做法显然不可取。

如果你以技术的角度来讨论这个问题，当然 SELECT 星号这种查询在不需要全部字段的情况下确实不好，但为什么不好我们下文再介绍。

但如果你以项目经理的角度来说，***SELECT 星号查询能大幅提升开发效率和减轻后续维护的时间***。
...
业务更新后，`users` 表新增了一个 `address` （住址）字段，而业务需求则是不仅要显示出 `name` 和 `email` 现在还要把住址也显示出来。
